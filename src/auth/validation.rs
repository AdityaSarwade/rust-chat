use crate::auth::constants::LenText;
use crate::auth::helpers::check_valid_name;
use crate::auth::helpers::check_valid_text;
use crate::auth::TypeValidDataFromRegistration::*;
use crate::auth::{TypeValidDataFromRegistration, TypeValidMail, TypeValidTwoStr};
...
pub fn get_valid_username_and_password(
    username: &str,
    password: &str,
    max_min_len_username: LenText,
    max_min_len_password: LenText,
) -> TypeValidTwoStr {
    if check_valid_text(username, max_min_len_username.max, max_min_len_username.min) {
    ...
    match get_valid_first_and_last_names(
        ...
        TypeValidTwoStr::Ok => {
            match get_valid_username_and_password(
                ...
                TypeValidTwoStr::BadFirst => BadUsername,
    ...
    match get_valid_first_and_last_names(
        ...
        TypeValidTwoStr::Ok => {
            match check_valid_text(
                &edit_model.username,
                max_min_len_username.max,
                max_min_len_username.min,
            ) {
                ...
                false => ValidEditModelError::BadUsername,
            }
        }
